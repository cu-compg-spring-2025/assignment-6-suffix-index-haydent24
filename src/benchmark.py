...
def benchmark_structures(reference_file, queries):
    ...
    print(f"Suffix tree built in {tree_build_time:.2f}s using {tree_build_mem:.2f}MB")
    ...
    print(f"Suffix array built in {array_build_time:.2f}s using {array_build_mem:.2f}MB")
    ...
    for length, queries_of_length in query_groups.items():
        ...
        for i, query in enumerate(queries_of_length):
            ...
            # Print detailed memory info every 10 queries
            if i % 10 == 0:
                print(f"    Tree search: {tree_search_time:.6f}s, {tree_search_mem:.6f}MB")
            ...
            # Print detailed memory info every 10 queries
            if i % 10 == 0:
                print(f"    Array search: {array_search_time:.6f}s, {array_search_mem:.6f}MB")
...
def plot_results(results, output_dir):
    ...
    # Create two separate memory usage plots
    plt.figure(figsize=(12, 10))
    
    # Tree memory
    plt.subplot(2, 1, 1)
    plt.plot(query_lengths, avg_search_mem['tree'], 's-', color='blue')
    plt.title('Suffix Tree: Average Search Memory Usage')
    plt.xlabel('Query Length')
    plt.ylabel('Memory (MB)')
    plt.grid(True)
    ...
    # Array memory
    plt.subplot(2, 1, 2)
    plt.plot(query_lengths, avg_search_mem['array'], '^-', color='red')
    plt.title('Suffix Array: Average Search Memory Usage')
    plt.xlabel('Query Length')
    plt.ylabel('Memory (MB)')
    plt.grid(True)
    
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'search_memory_separate.png'))
    plt.close()
    
    # Try a combined plot with log scale if memory values are very different
    plt.figure(figsize=(10, 6))
    ...
    plt.title('Average Search Memory Usage by Query Length (Log Scale)')
    plt.xlabel('Query Length')
    plt.ylabel('Memory (MB)')
    plt.yscale('log')  # Use logarithmic scale for y-axis
    plt.legend()
    plt.grid(True, which="both")  # Grid for both major and minor ticks
    plt.savefig(os.path.join(output_dir, 'search_memory_log_scale.png'))
    plt.close()
    ...
    for metric in ['time', 'memory']:
        ...
        for i, length in enumerate(query_lengths):
            table_data.append([
                length, 'Tree', 
                f"{tree_values[i][0]:.6f}", 
                f"{tree_values[i][1]:.6f}", 
                f"{tree_values[i][2]:.6f}", 
                f"{tree_values[i][3]:.6f}"
            ])
            table_data.append([
                '', 'Array', 
                f"{array_values[i][0]:.6f}", 
                f"{array_values[i][1]:.6f}", 
                f"{array_values[i][2]:.6f}", 
                f"{array_values[i][3]:.6f}"
            ])
    ...
    with open(os.path.join(output_dir, 'benchmark_results.txt'), 'w') as f:
        ...
        for structure in structures:
            f.write(f"{structure} - Time: {results[structure]['build']['time']:.6f}s, Memory: {results[structure]['build']['memory']:.6f}MB\n")
        ...
        for length in query_lengths:
            ...
            for structure in structures:
                ...
                f.write(f"{structure} - Time (s): Mean={np.mean(times):.6f}, Min={np.min(times):.6f}, "
                        f"Max={np.max(times):.6f}, StdDev={np.std(times):.6f}\n")
                        
                f.write(f"{structure} - Memory (MB): Mean={np.mean(mems):.6f}, Min={np.